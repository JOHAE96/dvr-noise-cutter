...
uv run dvr-noise-cutter cut <video.mp4> --output clean.mp4   # remove noise segments
```
...
- **`cutter.py`** — all ffmpeg/ffprobe orchestration, via plain `subprocess.run`, not `ffmpeg-python`.
  - `invert_segments` complements noise segments against total duration to get "keep" intervals (this, not the noise segments themselves, is what gets extracted).
  ...
  - `cut_noise_segments` is the entry point: does all work inside one `TemporaryDirectory`, concatenates via ffmpeg's concat demuxer (requires all intermediate segments to share codec parameters, which the pipeline guarantees by construction — either literal stream copies of the source, or re-encoded with identical fixed settings).

- **`cli.py`** — Typer app wiring `analyze`/`cut` to the above. `_run_detection` is the shared helper both commands call (avoid duplicating the `analyze_video_frames` + `segments_from_scores` pair). Progress reporting is driven by `analyze_video_frames`'s `on_frame_read` callback, not a separate pass over the video.
